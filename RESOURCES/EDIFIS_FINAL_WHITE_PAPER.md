...
Both feed the app's home screens and notifications (Laravel Reverb), so "what's next" is always current even when the wider internet is down (served from the local node).
